...
%%% RMSD:
rmsd(1) = sqrt(sum(  ( ( Cr-mean(Cr) )-( Cr-mean(Cr) )).^2)  /N);
rmsd(2) = sqrt(sum(  ( ( Cf-mean(Cf) )-( Cr-mean(Cr) )).^2)  /N);

%%% RMSE:
rmse(1) =  sqrt(sum(  ((Cr - Cr).^2)  /N)); %sqrt(mean((Cr - Cr).^2));
rmse(2) =  sqrt(sum(  ((Cf - Cr).^2)  /N)); %sqrt(mean((Cr - Cf).^2));

%%% R-squared:
rsq(1) = rsquared(Cr,Cr);
rsq(2) = rsquared(Cf,Cr);
...
%%bias
bias(1)=mean(Cr)-mean(Cr);
bias(2)=mean(Cf)-mean(Cr);
