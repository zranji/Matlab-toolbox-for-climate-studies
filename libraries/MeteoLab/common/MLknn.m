...
VnoNaN = find(nansum(double(isnan(V)),2)==0);
UnoNaN = find(nansum(double(isnan(U)),2)==0);
[ind1,dist1]=binMLknn(V(VnoNaN,:),U(UnoNaN,:),k,DistClass);
indAng(VnoNaN,:)=UnoNaN(ind1);
distAng(VnoNaN,:)=dist1;

VnoNaN=setdiff([1:Nest],VnoNaN);
VnoNaN=VnoNaN(:)';

for i=VnoNaN,
	switch lower(DistClass)
		case {'norm-2',2}
			aux=sqrt(nansum((repmat(V(i,:),Nestu,1)-U).^2,2));
		case {inf,'inf'}
		...
		otherwise
			if isnumeric(DistClass)
				aux=nansum(abs(repmat(V(i,:),Nestu,1)-U).^DistClass,2).^(1/DistClass);
			else
