...
if nargin < 2;error('stats:cmtest2:TooFewInputs','At least 2 inputs are required.');end
verbose=0;% vebosity of display
% Ensure each sample is a VECTOR.
[rows1,columns1]=size(x1);[rows2,columns2]=size(x2);
if (rows1~=1) & (columns1~=1),error('stats:cmtest2:VectorRequired','Sample X1 must be a vector.');end
if (rows2~=1) & (columns2~=1),error('stats:cmtest2:VectorRequired','Sample X2 must be a vector.');end
% Remove missing observations indicated by NaN's, and
% ensure that valid observations remain.
x1=x1(~isnan(x1));x1=x1(:);
x2=x2(~isnan(x2));x2=x2(:);
if isempty(x1),error('stats:cmtest2:NotEnoughData','Sample vector X1 is composed of all NaN''s.');end
if isempty(x2),error('stats:cmtest2:NotEnoughData','Sample vector X2 is composed of all NaN''s.');end
% Ensure the significance level, ALPHA, is a scalar between 0 and 1 and set default if necessary.
if (nargin >= 3) & ~isempty(alpha)
	if prod(size(alpha))>1,error('stats:cmtest2:BadAlpha','Significance level ALPHA must be a scalar.');end
	if (alpha <= 0 | alpha >= 1),error('stats:cmtest2:BadAlpha','Significance level ALPHA must be between 0 and 1.');end
else
	alpha=0.05;
end
% Calculate F1(x) and F2(x), the empirical (i.e., sample) CDFs.
binEdges=[-inf;sort([x1;x2]);inf];
binCounts1=histc(x1,binEdges);binCounts2=histc(x2,binEdges);
sumCounts1=cumsum(binCounts1)./sum(binCounts1);
sumCounts2=cumsum(binCounts2)./sum(binCounts2);
sampleCDF1=sumCounts1(1:end-1);sampleCDF2=sumCounts2(1:end-1);
N1=length(x1);N2=length(x2);N=N1+N2;
if verbose,fprintf('m1: %4.3f M1: %4.3f m2: %4.3f M2: %4.3f \n',min(sampleCDF1),max(sampleCDF1),min(sampleCDF2),max(sampleCDF2));end
...
z=[0.00000 0.02480 0.02878 0.03177 0.03430 0.03656 0.03865 0.04061 0.04247 0.04427 0.04601 0.04772 0.04939 0.05103 0.05265 0.05426 0.05586 0.05746 0.05904 0.06063 0.06222 0.06381 0.06541 0.06702 0.06863 ...
...
0.20939 0.21512 0.22114 0.22748 0.23417 0.24124 0.24874 0.25670 0.26520 0.27429 0.28406 0.29460 0.30603 0.31849 0.33217 0.34730 0.36421 0.38331 0.40520 0.43077 0.46136 0.49929 0.54885 0.61981 0.74346 1.16786 ]';
