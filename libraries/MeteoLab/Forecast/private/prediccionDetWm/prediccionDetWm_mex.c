...
void mexFunction(int nlhs, mxArray  *plhs[], int nrhs, const mxArray *prhs[]) {
	SDoubleMtx NAnlg[1],Anlg[1],Prdc[1],Dist[1],Ptnd[1],IndEx[1],NEx[1];
	SParamPrd Param[1];
	mxArray *Dum;
#ifdef VERBOSE_PRO
	...
	Umb->Dim=mxGetDimensions(Dum);
	if(Umb->Dim[0]<1) 
		mexErrMsgTxt("Error: Los Umbrales han de tener 1 filas");
	Umb->Mtx=mxGetPr(Dum);
	*/
...
#endif
	prediccionDetWm(Prdc,Anlg,Dist,Ptnd,Param);
	free(Prdc->Dim);	
}
