function [Y1]=prediccionDetNN(indAnlg,indDist,Y0,X0,X1,Parm)
...
nnet=newff([min(X0(:,1:nn),[],1);max(X0(:,1:nn),[],1)]',[2 2 1],{'tansig','tansig','purelin'},'trainMET');
...
for m=1:size(Y0,3)
   for k=1:size(Y0,2)
      for l=1:size(X1,1)   
         ...
         if sni >= minN
            [y0,mt,st]=prestd(Y0(ni(1:sni),k,m)');
            [x0,mp,sp]=prestd(X0(ni(1:sni),1:nn)');
            E=inf;
            V=NaN;
            for iT=1:5
            	nnet=init(nnet);
               ...
               if(EE<E)
                  V=poststd(sim(nnet,trastd(X1(l,1:nn)',mp,sp)),mt,st)';
                  E=EE;
               end
            end
            Y1(l,k,m)=V;
         ...
         if(~rem(l-1,10)),fprintf(1,'Dia=%d/%d\n',l,size(X1,1));end
      end
