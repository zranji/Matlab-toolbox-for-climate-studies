/* File:  grbseek.c 		based on Decoder's trqgetmsg() func;
   Revised by:
...
*
*    INTERFACE:
*       int grbseek (InFile, offset, Read_Index,  gh, errmsg)
*
...
*/
#if PROTOTYPE_NEEDED
int 	grbseek (	char *InFile,FILE **fp, long *offset, int Read_Index, 
			GRIB_HDR *gh, char *errmsg)
#else
int	grbseek (InFile,fp, offset, Read_Index,gh,errmsg)
			char *InFile; 
			FILE **fp;
			long *offset; 
			int Read_Index; 
...
*/
	...
	sprintf(errmsg,"%s %s: Got fseek error to pos= %ld\n",func,InFile,pos);
	status = 1; 
	goto DONE; 
	}
...
*/
	...
	{ 
	  if (nread<=4) {
	    ...
	    sprintf(errmsg,"%s %s: skip last %d bytes, too few for a Msg\n",
	    func, InFile, nread);
...
*/
	  ...
	  if (Read_Index &&  *offset != (bytenum + pos))  {
	      sprintf(errmsg,
	      "%s %s:   No Grib msg found at offset= %ld;  check Index File\n",
	      func, InFile, *offset);
...
*/
		...
		DPRINT3 (
		"%s: FSEEK %s error to pos+bytenum= %ld\n",
		func, InFile, pos+bytenum);
		sprintf(errmsg,
		"%s %s: FSEEK error to pos+bytenum= %ld\n",
		func, InFile, pos+bytenum);
		status= 1;
		goto DONE;
		}
...
*/
	  fwa_msg = (char *)gh->entire_msg;
	    ...
	    func, lMessageSize);
	    sprintf(errmsg,
            "%s: %s has truncated msg @offs=%ld (got %ld out of %ld bytes)\n",
            func, InFile, *offset, nread, lMessageSize);
	    status= 1;  /* to get out of Outer loop */
...
*/
		ptr += gh->pds_len;
	        if (ptr > end_ptr) {
		    sprintf(errmsg,
		    "%s %s:  corrupt PDSlen= %ld, Totlen=%ld, drop msg @%ld;\n"
		    , func,InFile,  gh->pds_len, gh->msg_length, *offset);
		    gh->pds_len= 0;  		     /* reset */
		    goto DROPMSG_N_LOOP;
		    }
...
*/
		...
		if (bit_set) {
		    ...
		    ptr += gh->gds_len;	/* bump PTR to sect*/
	             if (ptr > end_ptr) {
		       sprintf(errmsg,
     		       "%s %s: corrupt GDSlen= %ld, Totlen=%ld, drop msg @%ld\n"
		        , func,InFile,  gh->gds_len, gh->msg_length, *offset);
		        gh->gds_len= 0;  		/* reset */
		        goto DROPMSG_N_LOOP;
     		        }
...
*/
		...
		if (bit_set) {
		    ...
		    ptr += gh->bms_len;		/* bump PTR to sect */
	            if (ptr > end_ptr) {
		       sprintf(errmsg,
		       "%s %s: corrupt BMSlen= %ld, Totlen=%ld, drop msg @%ld\n"
			, func, InFile, gh->bms_len, gh->msg_length, *offset);
		        gh->bms_len= 0;  		/* reset */
		        goto DROPMSG_N_LOOP;
     		        }
...
*/
		...
		ptr += gh->bds_len;
	        if (ptr > end_ptr) {
		    sprintf(errmsg,
		    "%s %s:  corrupt BDSlen= %ld, Totlen=%ld, drop msg @%ld\n"
		    , func, InFile, gh->gds_len, gh->msg_length, *offset);
		    gh->gds_len= 0;  /* reset */
		    goto DROPMSG_N_LOOP;
		    }
...
*/
	...
	if (Read_Index) {
		sprintf(errmsg,
		"%s %s:  no 7777 found for msg at %ld, check indexfile\n",
		func, InFile, *offset);
		}
...
*/
     ...
     if (Read_Index) {  /* Catch 3 cases:
			...
			*/
	   sprintf(errmsg,
	   "%s %s:  No Grib Msg found at IndexFile's offset = %ld; "\
	   " Check Index File\n" , func, InFile, *offset);
