...
double ref_val;            /* reference value (minimum value) */
...
int sign;                 /* sign + or - */
double dscale;             /* 10 to the decimal scaling power */
double bscale;             /* 2 to the binary scaling power   */
...
double *grib_data=0;       /* local work array for grid data */
double fdata=0.;            /* data value stored in reference */
unsigned long i;         /* array counter */
...
*/
  ...
  ref_val = ( mantissa * (pow(16.0,c_ristic)) * (pow(2.0,-24.0)));
...
*/
  /* set binary scale */
  bscale = pow (2.0, scale);

  /* set decimal scale */
  dscale = pow (10.0,  deci_scale);
...
*/
  if (!data_width) {
     ...
     fdata = (ref_val / dscale);
     *pgrib_data = (double *) MYMALLOC (sizeof(double));
     **pgrib_data = fdata;
...
*           ENDIF
*/
  grib_data =(double *) MYMALLOC (data_pts * sizeof(double));
  if (grib_data==NULL) {
...
*/
  ...
  for(i=0;i < data_pts ;i++)  /* not DPRINTing here */
     ...
     grib_data[i]= (ref_val + (something * bscale))/dscale;
     }
