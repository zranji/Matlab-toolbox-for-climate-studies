...
int readgrib(char *fid,long *offset,float **grib_data,int *m,int *n,int *scan,MessageStruct *Mess){
	int nReturn;
	...
	if (Mess->bms.uslength>0)
      	if (nReturn=apply_bitmap(&Mess->bms, &grib_data[0], FILL_VALUE, &Mess->bdsHead))
	{
		prt_err(nReturn);  /* Print error code and message */
		printf("error=%d----->%d\n",nReturn,Mess->gds.head.usData_type);
#ifdef MATLAB_MEX_FILE
