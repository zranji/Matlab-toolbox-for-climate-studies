function Ypred = elmTest(X,Y,indsTrain,indsTest,method,model,XDataCluster)
...
if strcmp(prdType,'PC')
	X = X(:,1:ncps);
	Ypred = NaN*zeros(length(indsTest),size(model.MODEL,2));
	for c=1:prod(clustering.NumberCenters)
		ii0 = find(Xcluster==c);
		[ii,i1,i2] = intersect(ii0,indsTest);
		for k=1:size(Ypred,2)
			ixCols = [1:size(X,2)];
			if ~isempty(Feats)
				ixCols = Feats{c,k};
			end
			Ypred(i2,k) = downELMVal(model.MODEL{c,k},X(ii,ixCols));
		end
	end
else
	...
	for c=1:prod(clustering.NumberCenters)
		...
		for k=1:size(Y,2)
			...
			nnAverage=[];
			if nnAve
				nnAverage=repmat(NaN,size(ii,2),size(model.dmn.par,1));
				for i=1:size(model.dmn.par,1)
			...
			Ypred(i2,k) = downELMVal(model.MODEL{c,k},[X(ii,indsVecinos), nnAverage(:,colsAve)]);
		end
	end
end
