...
hiddenLayersType = 'tansig';
if isfield(method.properties,'TypeOfHiddenLayers')
	hiddenLayersType = method.properties.TypeOfHiddenLayers;
end
if isfield(method.properties,'NumberOfHiddenLayers')
	hiddenLayersNumber = str2num(method.properties.NumberOfHiddenLayers);
end
Feats = {}; FeatsOrder = {};
...
if strcmp(prdType,'PC')
	...
	for c=1:prod(clustering.NumberCenters)
		...
		for ss=1:size(Y,2)
			...
			sizes = repmat(NaN,1,hiddenLayersNumber);
			sizes(1) = length(ixCols);            
			for sx=2:length(sizes)
				sizes(sx) = floor(sizes(sx-1)/2);
			end
			sizes = sizes(sizes>1);
			types = {length(sizes)+1};
			for sx=1:length(sizes)
				types{sx} = hiddenLayersType;
			end
			types{end} = 'purelin';
			mynet = newff(minmax(X(iinew,ixCols)'),minmax(Y(iinew,ss)'),sizes,types);
			Beta{c,ss} = train(mynet,X(iinew,ixCols)',Y(iinew,ss)');
		end
	end
else
	Beta = {};
	...
	for c=1:prod(clustering.NumberCenters)
		...
		for k=1:size(Y,2)
			...
			sizes = repmat(NaN,1,hiddenLayersNumber);
			if nnAve
				sizes(1) = length(indsVecinos)+length(colsAve);
			else
				sizes(1) = length(indsVecinos);
			end
			for sx=2:length(sizes)
				sizes(sx) = floor(sizes(sx-1)/2);
			end
			...
			types{end} = 'purelin';
			mynet = newff(minmax([X(iinew,indsVecinos) nnAverage(:,colsAve)]'),minmax(Y(iinew,k)'),sizes,types);
			Beta{c,k} = train(mynet,[X(iinew,indsVecinos) nnAverage(:,colsAve)]',Y(iinew,k)');
		end
	end
end
if ~isempty(Feats)
