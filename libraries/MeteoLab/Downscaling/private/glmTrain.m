function MODEL = glmTrain(X,Y,indsTrain,indsTest,method,model,XDataCluster)

if isfield(method.properties,'ThresholdPrecip')
    method.properties.ThresholdPrecip = str2num(method.properties.ThresholdPrecip);
else
...
if strcmp(prdType,'PC')
    ...
    for c=1:prod(clustering.NumberCenters)
        ...
        for ss=1:size(Y,2)
            ...
            MODEL.occModel{c,ss} = downOccFit(X(iinew,ixCols),Y(iinew,ss),1:length(iinew),'umb',method.properties.ThresholdPrecip);
            if isfield(method.properties, 'minrainydays')
                MODEL.amoModel{c,ss} = downGLMFit(X(iinew,ixCols),Y(iinew,ss),1:length(iinew),'umb',method.properties.ThresholdPrecip,'minrainydays',method.properties.minrainydays);
            else
                MODEL.amoModel{c,ss} = downGLMFit(X(iinew,ixCols),Y(iinew,ss),1:length(iinew),'umb',method.properties.ThresholdPrecip);
            end
        end
    end
else
    ...
    for c=1:prod(clustering.NumberCenters)
        ...
        for k=1:size(Y,2)
            ...
            MODEL.occModel{c,k} = downOccFit([X(iinew,indsVecinos) nnAverage(:,colsAve)],Y(iinew,k),1:length(iinew),'umb',method.properties.ThresholdPrecip);
            if isfield(method.properties, 'minrainydays')
                MODEL.amoModel{c,k} = downGLMFit([X(iinew,indsVecinos) nnAverage(:,colsAve)],Y(iinew,k),1:length(iinew),'umb',method.properties.ThresholdPrecip,'minrainydays',method.properties.minrainydays);
            else
                MODEL.amoModel{c,k} = downGLMFit([X(iinew,indsVecinos) nnAverage(:,colsAve)],Y(iinew,k),1:length(iinew),'umb',method.properties.ThresholdPrecip);
            end
            clear indsVecinos nnAverage
        end
    end
end
if ~isempty(Feats)
