...
%   operation.  The resulting contours are returned in a cell arrays of x and
%   y coordinates.  Each contour is represented as a cell element.  There are
%   no holes from the simple polygon intersection operation.

%  Written by:  A. Kim
%  Copyright 1996-2002 Systems Planning and Analysis, Inc. and The MathWorks, Inc.
%  $Revision: 1.3 $ $Date: 2002/03/20 21:26:55 $
...
% if polygons are similar, intersection is either polygon
	xpi = {x1};  ypi = {y1};  return
elseif strcmp(p1,'in') & strcmp(p2,'out')
% if polygon 1 is inside of polygon 2, intersection is polygon 1
	xpi = {x1};  ypi = {y1};  return
elseif strcmp(p1,'out') & strcmp(p2,'in')
% if polygon 2 is inside of polygon 1, intersection is polygon 2
	xpi = {x2};  ypi = {y2};  return
elseif strcmp(p1,'out') & strcmp(p2,'out')
% if both polygons are outside of each other, no intersection
	xpi = {};  ypi = {};  return
end
...
% record path traversed thus far
		x = [x; x1(indx1); x2(indx2)];
		y = [y; y1(indx1); y2(indx2)];
